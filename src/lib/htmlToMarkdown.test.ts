...
describe('insertAtSelection', () => {
    ...
    it('replaces the active selection and moves the caret after the inserted text', () => {
        ...
        let nextValue = '';
        insertAtSelection(textarea, 'SNS Publish', (value) => {
            nextValue = value;
            textarea.value = value;
        });

        expect(nextValue).toBe('hello SNS Publish');

        vi.runAllTimers();

        expect(textarea.selectionStart).toBe('hello SNS Publish'.length);
        expect(textarea.selectionEnd).toBe('hello SNS Publish'.length);
    });
});
