...
turndownService.addRule('image', {
    ...
    replacement: (_content, node: any) => {
        ...
        let src = (node.getAttribute?.('src') || node.src || '').trim();
        const title = (node.title || '').replace(/"/g, '\\"');

        if (!src) return '';

        // Store data-URLs in the image store to keep the editor lightweight.
        ...
        Promise.all(imageFiles.map(fileToDataUrl))
            .then((dataUrls) => {
                const markdownImages = dataUrls
                    .filter(Boolean)
                    .map((src, index) => {
                        const ref = storeImage(src);
                        return `![图片${dataUrls.length > 1 ? ` ${index + 1}` : ''}](${ref})`;
                    })
                    .join('\n\n');
